  ...
  try {
    ...
    analyzeJobDescription(job.description, { model: aiModel })
      .then(async (analysis) => {
        ...
        console.log(`[jobs] Re-analysis complete for job ${jobId} with model ${aiModel}`);
        return runAutoPipeline(jobId);
      })
      .catch((err) => {
        console.error(`[jobs] Re-analysis failed for job ${jobId}:`, err);
        prisma.job
          .update({
            where: { id: jobId },
    ...
    console.error("Job re-analyze error:", error);
    return NextResponse.json(
      { error: "Failed to re-analyze job" },
      { status: 500 }
    );
  }
}
